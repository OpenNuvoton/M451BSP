...
void GPIO_Init(void)
{
    /* Enable PE.12 interrupt by falling edge trigger */
    PE->INTTYPE |= (GPIO_INTTYPE_EDGE << GPIO_INTTYPE_TYPE2_Pos);
    PE->INTEN |= GPIO_INTEN_FLIEN12_Msk;
...
void GPE_IRQHandler(void)
{
    ...
    if(GPIO_GET_INT_FLAG(PE, BIT12))
    {
        GPIO_CLR_INT_FLAG(PE, BIT12);
