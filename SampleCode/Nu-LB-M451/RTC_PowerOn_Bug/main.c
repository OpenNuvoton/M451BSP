...
//    ILI9341_LCD_acFont(2*24, 0, 7, White, Black);
    ...
    sprintf(sbuf, "RTC Test with Core Power On/Off");
    ILI9341_LCD_PutString(0, 0, sbuf, White, Black);
    if((SystemCoreClock%1000000) == 0)
    {
        sprintf(sbuf, "CPU @ %dMHz", SystemCoreClock/1000000);
    }
    else
    {
        sprintf(sbuf, "CPU @ %dHz", SystemCoreClock);
    }
    ILI9341_LCD_PutString(16, 0, sbuf, White, Black);
    ...
    /* Initial RTC and stay in normal state */
        ...
        u32Reg = M32(0x20007000);
        
        if(u32Reg != 0x55aa55aa)
        {
            M32(0x20007000) = 0x55aa55aa;
        }
        else
        {
            ...
            RTC->TICK    = RTC_TICK_1_SEC;   /* One RTC tick is 1/4 second */
        }
    ...
    sprintf(sbuf, "0x%08x  0x%08x", RTC->CAL, RTC->TIME);
    ILI9341_LCD_PutString(48, 0, sbuf, White, Black);
    ...
    while(1)
    {
        ...
        sprintf(sbuf, "20%02x/%02x/%02x %02x:%02x:%02x",
        ...
        ILI9341_LCD_PutString(32, 0, sbuf, White, Black);
