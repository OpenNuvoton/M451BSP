...
uint32_t g_au32UsbTmpBuf[((PLAY_RATE > REC_RATE) ? PLAY_RATE : REC_RATE) / 2000 * ((PLAY_CHANNELS > REC_CHANNELS) ? PLAY_CHANNELS : REC_CHANNELS)+24] = {0};
...
volatile uint32_t g_u32SampleRate;
...
/**
 * @brief       EP5 Handler (Iso IN feedback interrupt handler)
 *
 * @param[in]   None
 *
 * @return      None
...
void EP5_Handler(void)
{
    ...
    /* Get the address in USB buffer */
    pu8Buf = (uint8_t *)((uint32_t)USBD_BUF_BASE + USBD_GET_EP_BUF_ADDR(EP5));
...
void UAC_Init(void)
{
    ...
    /* EP5 ==> Isochronous IN feedback endpoint, address 5 */
    USBD_CONFIG_EP(EP5, USBD_CFG_EPMODE_IN | ISO_IN_FB_EP_NUM | USBD_CFG_TYPE_ISO);
    ...
    pu8Buf = (uint8_t *)((uint32_t)USBD_BUF_BASE + USBD_GET_EP_BUF_ADDR(EP5));
    /* Prepare the data to USB IN buffer */
    g_u32SampleRate = PLAY_LEN_FRAME;
    *pu8Buf++ = 0x00;
    *pu8Buf++ = (g_u32SampleRate & 0x3) << 6;
    *pu8Buf   = (g_u32SampleRate & 0xFC) >> 2;
...
void UAC_DeviceDisable(uint8_t u8Object)
{
    if(u8Object ==  UAC_MICROPHONE)
    {
        ...
        I2S_DisableInt(SPI1, I2S_FIFO_RXTH_INT_MASK);
        I2S_DISABLE_RX(SPI1);
        I2S_CLR_RX_FIFO(SPI1);
    }
    else
    {
        ...
        I2S_DisableInt(SPI1, I2S_FIFO_TXTH_INT_MASK);
        I2S_DISABLE_TX(SPI1);
        I2S_CLR_TX_FIFO(SPI1);
    }
}
...
void AdjFreq(void)
{
    ...
    if(g_i32AdjFlag == 0)
    {
        ...
        if(u32Size > (BUF_LEN * 3 / 4))
        {
            ...
            g_u32SampleRate = PLAY_LEN_FRAME - 1;
            g_i32AdjFlag = -1;
        }
    ...
    if((i32PreFlag != g_i32AdjFlag) || (i32Cnt++ > 40000))
    {
        ...
        //printf("%d %d %d %d\n", g_i32AdjFlag, u32Size/BUF_LEN, g_usbd_PlayVolumeL, g_usbd_RecVolumeL);
        printf("%d %d%% %d %d\n", g_u32SampleRate, (u32Size * 100) / 384, u32Size, BUF_LEN);
        i32Cnt = 0;
    }    
#else    
    ...
    if((i32PreFlag != g_i32AdjFlag) || (i32Cnt++ > 40000))
    {
        i32PreFlag = g_i32AdjFlag;
        //printf("%d %d %d %d\n", g_i32AdjFlag, u32Size/BUF_LEN, g_usbd_PlayVolumeL, g_usbd_RecVolumeL);
        printf("%d %d%% %d %d\n", g_i32AdjFlag, (u32Size * 100) / 384, u32Size, BUF_LEN);
        i32Cnt = 0;
    }
#endif
